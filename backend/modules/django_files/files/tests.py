...
class FileUploadTestCases(APITestCase):
    def setUp(self):
        self.user = User.objects.create_user(
            username="john", email="john12@gmail.com", password="john123@"
        )
    ...
    def test_upload_files_with_image_base64(self):
        ...
        image_1 = SimpleUploadedFile("cnic_image.png", image.getvalue())
        encoded_file = (
            f"data:image/png;base64,{base64.b64encode(image_1.read()).decode()}"
        )
        data = {
            "title": "my new image",
            "description": "description",
    ...
    def test_upload_files_without_description(self):
        ...
        image.seek(0)
        image_1 = SimpleUploadedFile("cnic_image.png", image.getvalue())
        data = {"title": "my new image", "user": self.user.id, "file": image_1}
    ...
    def test_upload_files_without_title(self):
        ...
        image.seek(0)
        image_1 = SimpleUploadedFile("cnic_image.png", image.getvalue())
        data = {"description": "description", "user": self.user.id, "file": image_1}
        response = self.client.post(url, data=data, format="multipart")
        self.assertEqual(response.status_code, status.HTTP_400_BAD_REQUEST)
    ...
    def test_upload_files_without_user(self):
        ...
        image.seek(0)
        image_1 = SimpleUploadedFile("cnic_image.png", image.getvalue())
        data = {"title": "my new image", "description": "description", "file": image_1}
        response = self.client.post(url, data=data, format="multipart")
        self.assertEqual(response.status_code, status.HTTP_400_BAD_REQUEST)
...
class UpdateSingleFileUploadTest(APITestCase):
    def setUp(self):
        ...
        self.fileUpload = FileUpload.objects.create(
            title="CB", description="About CB", user=self.user, file=self.image_1
        )
    ...
    def test_Valid_update_fileUpload(self):
        response = self.client.put(
            f"/modules/files/uploads/{self.fileUpload.id}/", data=self.valid_payload
        )
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        self.assertEqual(response.data["title"], self.valid_payload["title"])
        self.assertEqual(
            response.data["description"], self.valid_payload["description"]
        )
    ...
    def test_Invalid_update_fileUpload(self):
        ...
        response = self.client.put(
            f"/modules/files/uploads/{self.fileUpload.id}/", data=data
        )
...
class DeleteSingleAppointment(APITestCase):
    def setUp(self):
        ...
        self.fileUpload = FileUpload.objects.create(
            title="CB", description="About CB", user=self.user, file=self.f
        )
